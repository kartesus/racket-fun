...
(define (update-priority s p)
        (let ([q (for/first ([x (in-heap queue)] #:when (equal? s (car x)))
                            x)])
...
(define (queue->list)
        (for/list ([n (in-heap queue)])
                  n))

(define (in-queue? s)
        (for/or ([x (in-heap queue)])
                (equal? (car x) s)))
...
(define (init-graph start-node edges)
        (let* ([INFINITY 9999]
              ...
              (hash-clear! lengths)
              (for ([e all-edges])
                   (hash-set! lengths (cons (first e) (second e)) (third e)))
